...
def diffusion(
    ...
    precond_func, R = make_regularizer(
        regularizer, n, m, D, hypergraph, weights)

    if verbose > 0:
        # print(f"Average degree = {sum(D) / n:.3f}. Average rank = {sum(rank) / m:.3f}")
        total_deg = float(D.sum())          # D, rank 都是 1-d ndarray
        total_rank = float(rank.sum())
        if verbose > 0 and m:                # m==0 → 只印 degree
            print(f"Average degree = {total_deg / n:.3f}. "
                  f"Average rank = {total_rank / m:.3f}")
